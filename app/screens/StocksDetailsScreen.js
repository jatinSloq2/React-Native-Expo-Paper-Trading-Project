...
const getCryptoLogoUrl = (symbol) => {
    ...
    // First: try known high-quality logos from your list
    const known = EXTENDED_SYMBOL_INFO[symbol];
    if (known?.image) {
        return known.image;
    }

    // Second: try common aliases
    const aliases = {
        bnb: 'bnb',
        matic: 'polygon',
        poly: 'polygon',
    ...
    // Final fallback: cryptologos.cc (works for almost everything)
...
export default function CryptoDetailsScreen() {
    ...
    const symbolInfo = (() => {
        const hardcoded = EXTENDED_SYMBOL_INFO[stock.symbol];
        if (hardcoded) {
            return {
                name: hardcoded.name,
        ...
        // Dynamic fallback
        const baseAsset = stock.symbol.replace('USDT', '');
        return {
            name: baseAsset,
            color: '#2E5CFF',
            image: getCryptoLogoUrl(stock.symbol)  // This will almost always work
        };
    })();
